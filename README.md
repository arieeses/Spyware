...
**增量 1 · 检测核心**(本目录 `spyware/`)——纯 Python 标准库,零外部依赖。
...
- 面板机 Nginx 配 `spyware` 日志格式 + `real_ip`;订阅网关按 `/api/decision` 分级下发。

## 本地试用 / 可视化控制后台

```bash
python3 -m spyware.web            # 打开 http://127.0.0.1:8787
```

**首次打开**会跳到 `/register` 创建管理员账号,之后用账号密码登录。

- **登录 / 注册 / 忘记密码**:忘记密码时,重置链接**输出到服务器控制台**(自托管安全方式);也可运行 `python3 -m spyware.web resetpw <用户名>` 直接改密。
- **数据库迁移**:单文件 SQLite,系统设置页可**备份下载**;迁移只需拷 `spyware.db`,或设 `SPYWARE_DB=/路径/spyware.db` / config.json 的 `db_path`。
- **自动升级**:系统设置 › 检查更新,git 部署可**点击 `git pull` 并自动重启**。

侧边栏分类导航(aaPanel 风格):仪表盘 / 接入管理(v2board·1Panel·aaPanel)/ 风险管理 / 运行 / 系统。面板功能:

- **数据源管理**:增 / 删 / 启停 日志源(Nginx spyware 日志)和 v2board(MySQL 只读)
- **手动运行**:单源「运行」或「▶ 手动运行全部」
...
> 换端口:`--port 9000`;停止:`pkill -f spyware.web`。

## 命令行(等价能力, 便于 cron)

```bash
python3 -m spyware.cli ingest --log sample/sub.log   # 增量导入(cron 安全)
python3 -m spyware.cli load-users --users sample/users.json
python3 -m spyware.cli sync-v2board                  # 需 config.json
python3 -m spyware.cli analyze                        # --all / --level 高 中
```
...
- `spyware/config.py` —— 权重与阈值(按真实数据校准)
